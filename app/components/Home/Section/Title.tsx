...
// Shadow component
const ShadowComp = ({ children }) => children

/**
 * Lazy load component
 */
const useLazyComponent = (importer, enabled) => {
  const [Component, setComponent] = React.useState(() => ShadowComp)

  React.useEffect(() => {
    if (enabled) {
      importer().then(({ default: LazyComponent }) =>
